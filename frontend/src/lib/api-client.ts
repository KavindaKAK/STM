...
async function apiRequest<T>(
    ...
    const fullUrl = API_URL === '/api' ? endpoint : `${API_URL}${endpoint}`;
...
export const api = {
    ...
    // Admin - Shipping Fees
    createShippingFee: (data: any, token: string) => apiRequest('/api/admin/shipping-fees', { method: 'POST', body: JSON.stringify(data), token }),
    updateShippingFee: (id: string, data: any, token: string) => apiRequest(`/api/admin/shipping-fees/${id}`, { method: 'PUT', body: JSON.stringify(data), token }),
    deleteShippingFee: (id: string, token: string) => apiRequest(`/api/admin/shipping-fees/${id}`, { method: 'DELETE', token }),
};
