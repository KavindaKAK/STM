...
async function seed() {
    try {
        ...
        const existingFees = await ShippingFee_1.default.countDocuments();
        if (existingFees === 0) {
            const defaultFee = 500; // Default LKR 500
            const shippingFees = types_1.SRI_LANKA_DISTRICTS.map((district) => ({
                district,
                feeLkr: district === 'Colombo' ? 300 : defaultFee, // Lower fee for Colombo
            }));
            await ShippingFee_1.default.insertMany(shippingFees);
            console.log(`✅ Seeded shipping fees for ${types_1.SRI_LANKA_DISTRICTS.length} districts`);
        }
        else {
            console.log('ℹ️  Shipping fees already exist');
        }
        // Seed sample brands
        const existingBrands = await Brand_1.default.countDocuments();
        if (existingBrands === 0) {
            const sampleBrands = [
                // Tyre Brands
            ...
            await Brand_1.default.insertMany(sampleBrands);
        ...
        // Seed sample banners
        const existingBanners = await Banner_1.default.countDocuments();
        if (existingBanners === 0) {
            ...
            await Banner_1.default.insertMany(sampleBanners);
        ...
        console.log(`📍 Districts Seeded: ${types_1.SRI_LANKA_DISTRICTS.length} districts`);
        console.log(`   ${types_1.SRI_LANKA_DISTRICTS.join(', ')}\n`);
